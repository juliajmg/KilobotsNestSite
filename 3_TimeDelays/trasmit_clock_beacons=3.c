  ...
  if(kilo_uid == BEACON_1){
    ...
    if(kilo_ticks >= (mydata->t + (DELTA_T))){
     ...
     if((mydata->my_id == BEACON_2) | (mydata->my_id == BEACON_3)){
       if((mydata->bot_id == BEACON_1) & (mydata->bot_cycle != mydata->cycle)){
         mydata->cycle = mydata->bot_cycle;
         //update_message();
     }
   }
      else {
        if((mydata->my_id != BEACON_2) & (mydata->my_id != BEACON_3) & ((mydata->bot_id == BEACON_1) | (mydata->bot_id == BEACON_2) | (mydata->bot_id == BEACON_3)) & (mydata->bot_cycle != mydata->cycle)){
          mydata->cycle = mydata->bot_cycle;
