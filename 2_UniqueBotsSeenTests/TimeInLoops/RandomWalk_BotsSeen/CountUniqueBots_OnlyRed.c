...
void bots_seen(){
  if(mydata->my_id != BEACON_2 && mydata->my_id != BEACON_3){
    if((mydata->bots_seen >= 0) & (mydata->bots_seen < 4)){
      set_color(RGB(1,0,0));
    }
    if((mydata->bots_seen >=4) & (mydata->bots_seen < 8)) {
      set_color(RGB(0,1,0));
    }
    if((mydata->bots_seen >= 8) & (mydata->bots_seen < 12)){
      set_color(RGB(0,0,1));
    }
...
void setup()
{
  ...
  mydata->color_flag = mydata->dance_time = 0;
    ...
    if(mydata->bot_cycle > mydata->cycle){
      ...
      if((mydata->my_id != BEACON_2) && (mydata->my_id != BEACON_3)){
        ...
        if(mydata->dancing == NO_DANCE){
          ...
          decide_dance();
          bots_seen();
          mydata->color_flag = mydata->dance_time;
          reset_parameters();
