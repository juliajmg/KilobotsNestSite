  ...
  void loop()
  {
    ...
    if(mydata->bot_cycle < 30){
      ...
      if(mydata->p < 200){ //quitar esto que es redundante
    ...
    if(mydata->my_id != BEACON_2 && mydata->my_id != BEACON_3){
      ...
      if((mydata->bots_seen >= 3) & (mydata->bots_seen < 7)){
        set_color(RGB(1,0,0));
      }
      if((mydata->bots_seen >=7) & (mydata->bots_seen < 11)) {
        set_color(RGB(0,1,0));
      }
      if((mydata->bots_seen >= 11) & (mydata->bots_seen < 14)){
        set_color(RGB(0,0,1));
      }
      if(mydata->bots_seen >= 14){
        set_color(RGB(1,1,1));
      }
    }
  }
