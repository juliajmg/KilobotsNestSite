...
void loop()
{
  ...
  update_message();
  if(mydata->my_id != BEACON_2 && mydata->my_id != BEACON_3){
    if(mydata->bots_seen < 6){
      set_color(RGB(0,0,0));
    }
    if((mydata->bots_seen >= 6) & (mydata->bots_seen < 12)){
      set_color(RGB(1,0,0));
    }
    if((mydata->bots_seen >=12) & (mydata->bots_seen < 18)) {
      set_color(RGB(0,1,0));
    }
    if((mydata->bots_seen >= 18) & (mydata->bots_seen < 24)){
      set_color(RGB(0,0,1));
    }
    if(mydata->bots_seen >= 24){
      set_color(RGB(1,1,1));
    }
  }
  #ifdef DEBUG
